...
d'une app `nextjs-passenger` (voir ADR-0003) :
...
1. **Setup Node.js App** → *Create Application*
   ...
   - Application root : le chemin qui contiendra `current` (ex.
     `peci.org` si l'app root cPanel est `/home/user/peci.org`, et que
     `deploy_path` du workflow pointe vers ce même dossier).
   - Application URL : le (sous-)domaine cible.
   - **Application startup file** : `current/server.js`
...
3. Cliquer **Create** — cPanel crée l'app et le dossier `tmp/`. Le tout
   premier déploiement (fait par le workflow) créera `current` et pourra
...
(sous-)domaine (cPanel → Domains) pointe vers
